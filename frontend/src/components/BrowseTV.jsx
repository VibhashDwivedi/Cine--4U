...
const BrowseMovies = () => {
    ...
    const getMovies = async () => {
        ...
        const res = await fetch(`https://api.themoviedb.org/3/search/tv?api_key=3b813a8cad6a0e292c66f061d98d42a8&query=${searchQuery}`);
        if (res.status === 200) {
    ...
    const displayMovies = () => {
        ...
        return movieList.map((movie) => (
            ...
            <img src={`https://image.tmdb.org/t/p/w500/${movie.poster_path}`} height={270} className="card-img-top" alt="..."/></Link>
             ...
             <Link to={`/tv/${movie.id}`} style={{textDecoration:'none'}} ><h5 className="text-dark " >{movie.name}</h5></Link>
      ...
      <Navbar/>
        ...
        <div className="center col-lg-10 col-md-10 ">
          <div className="row">
            ...
            <h1 className='text-center m-4 mx-2'>Browse TV Shows</h1>
            </div>
            <div className="col-md-4">
                <Link to='/browsemovies' className='ms-auto'>
                <button className="btn btn-primary m-4  float-end switch-btn">Movies</button>
                </Link>
            </div>
         </div>  
